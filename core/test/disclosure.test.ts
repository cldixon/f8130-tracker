...
describe('building a disclosure', () => {
  test('reveals only the requested field', () => {
    ...
    const d = buildDisclosure({ bundle, fields: ['costCents'] })

    assert.equal(d.fields.length, 1)
    assert.equal(d.fields[0]!.field, 'costCents')
    assert.equal(d.fields[0]!.value, overhaulForm.costCents)

    // The document a lessor would receive must not contain the commercially
    // sensitive fields it was not given.
    const json = JSON.stringify(d)
    assert.ok(!json.includes('Metering valve wear'), 'findings leaked')
    assert.ok(!json.includes('Example Air'), 'customer leaked')
    assert.ok(!json.includes('Full overhaul per CMM'), 'workscope leaked')
  })

  test('carries exactly one nonce, not fifteen', () => {
    const { bundle } = fixture()
    const d = buildDisclosure({ bundle, fields: ['costCents'] })
    const json = JSON.stringify(d)
    const leaked = bundle.nonces.filter((n) => json.includes(n))
    assert.deepEqual(leaked, [bundle.nonces[FIELD_ORDER.indexOf('costCents')]])
  })
  ...
  test('names the withheld fields without valuing them', () => {
    ...
    assert.equal(result.withheld.length, 14)
    assert.ok(result.withheld.includes('findings'))
    assert.ok(!result.withheld.includes('costCents'))
  })
  ...
  test('multiple fields verify together', () => {
    ...
    const d = buildDisclosure({
      bundle,
      fields: ['costCents', 'status', 'completedAt'],
    })
    const result = verifyDisclosure(d, commitment.root)
  ...
  test('field order is normalized, so requests are order-independent', () => {
    const { bundle } = fixture()
    const a = buildDisclosure({ bundle, fields: ['completedAt', 'costCents'] })
    const b = buildDisclosure({ bundle, fields: ['costCents', 'completedAt'] })
    assert.deepEqual(
...
describe('verifying a disclosure', () => {
  ...
  test('a proof against a different record fails', () => {
    const { bundle } = fixture()
    const other = commitForm({ ...overhaulForm, costCents: 999 })
    const d = buildDisclosure({ bundle, fields: ['costCents'] })
  ...
  test('one bad field fails the whole disclosure', () => {
    const { commitment, bundle } = fixture()
    const d = buildDisclosure({ bundle, fields: ['costCents', 'status'] })
...
describe('what a disclosure leaks', () => {
  test('sibling hashes are exposed but reveal nothing', () => {
    ...
    // Four sibling hashes for a 16-leaf tree. They are unavoidable — they are
    ...
    for (const value of ['OVERHAULED', 'NEW', 'Example Air', '1']) {
      ...
      assert.ok(!json.includes('Metering'))
    }
  })
  ...
  test('withholding is visible: the verifier knows what it was not shown', () => {
    ...
    assert.equal(result.withheld.length + result.fields.length, 15)
  })
})
...
describe('parsing untrusted disclosures', () => {
  test('rejects a mismatched field index', () => {
    const { bundle } = fixture()
    const d = JSON.parse(
      JSON.stringify(buildDisclosure({ bundle, fields: ['costCents'] })),
    )
    d.fields[0].index = 0
  ...
  test('builds straight from bundle JSON', () => {
    const { commitment, bundle } = fixture()
    const d = discloseFromJson(JSON.parse(JSON.stringify(bundle)), ['costCents'])
